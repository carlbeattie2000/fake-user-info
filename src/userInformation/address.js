...
function postcode() {
  ...
  const getArrItem = (arr) => arr[globalFunctions.randomMinMax(0, arr.length - 1)];
  const getNum = () => globalFunctions.randomMinMax(1, 9);

  return getArrItem(districtCodes) + getNum() + getArrItem(alphabet);
}

function streetName() {
  let streetName = globalFunctions.getJSONContentArray("streetNames.json", __DATA_PATH);
...
function city() {
  let city = globalFunctions.getJSONContentArray("cities.json", __DATA_PATH);
  return city[globalFunctions.randomMinMax(0, city.length - 1)];
}

function county() {
  let county = globalFunctions.getJSONContentArray("counties.json", __DATA_PATH);
