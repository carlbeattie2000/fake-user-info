...
test("finance banking tests", async (t) => {
  ...
  await t.test(
    "Banking expiry date should return a string that matches for format MM/YY",
    () => {
  ...
  await t.test(
    "Banking balance should equal a float, and be not greater than the max passed. Should also return a string if bool is passed",
    () => {
      const floatBalance = impostorInstance.finance.banking.balance({
        max: 5000,
      });
      const stringBalance = impostorInstance.finance.banking.balance({
        formattedString: true,
      });

      equal(typeof floatBalance, typeof 0.5);
...
test("finance transaction tests", async (t) => {
  await t.test("transactionString should return a string", () => {
    const transactionString =
      impostorInstance.finance.banking.transactionString();
  ...
  await t.test("transactionObject should return a object", () => {
    const transactionObject =
      impostorInstance.finance.banking.transactionObject();
  ...
  await t.test("statementString should return an array ", () => {
    const statementStrings =
      impostorInstance.finance.banking.statementString(5);
  ...
  await t.test("statementString every item should be a string", () => {
    ...
    equal(
      statementStrings.every((string) => typeof string === typeof ""),
      true,
    );
  });
  ...
  await t.test("statementObject should return an array", () => {
    const statementObjects =
      impostorInstance.finance.banking.statementObject(5);
  ...
  await t.test("statementObject every item should be a object", () => {
    ...
    equal(
      statementObjects.every((object) => typeof object === typeof {}),
      true,
    );
  });
});
...
test("finance crypto tests", async (t) => {
  ...
  await t.test("bitcoinPrivateKey should return a string", () => {
    const bitcoinPrivateKey =
      impostorInstance.finance.crypto.bitcoinPrivateKey();
